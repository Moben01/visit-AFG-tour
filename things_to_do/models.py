...
class Top_things_to_do_in_province(models.Model):
    PROVINCE_CHOICES = [
        ('Badakhshan', 'Badakhshan'), ('Badghis', 'Badghis'), ('Baghlan', 'Baghlan'),
        ('Balkh', 'Balkh'), ('Bamyan', 'Bamyan'), ('Daykundi', 'Daykundi'),
        ('Farah', 'Farah'), ('Faryab', 'Faryab'), ('Ghazni', 'Ghazni'),
...
class Top_things_to_do_in_province_images(models.Model):
    place = models.ForeignKey(Top_things_to_do_in_province, on_delete=models.CASCADE, related_name='images')
    image = models.ImageField(upload_to='things-to-do/more/')
    caption = models.CharField(max_length=255, blank=True)
...
class Popular_Tourist(models.Model):
    PROVINCE_CHOICES = [
        ('Badakhshan', 'Badakhshan'), ('Badghis', 'Badghis'), ('Baghlan', 'Baghlan'),
        ('Balkh', 'Balkh'), ('Bamyan', 'Bamyan'), ('Daykundi', 'Daykundi'),
        ('Farah', 'Farah'), ('Faryab', 'Faryab'), ('Ghazni', 'Ghazni'),
...
class Popular_Tourist_images(models.Model):
    place = models.ForeignKey(Popular_Tourist, on_delete=models.CASCADE, related_name='images')
    image = models.ImageField(upload_to='place_images/more/')
    caption = models.CharField(max_length=255, blank=True)

    def __str__(self):
        return f"Image of {self.place.title}"     # Inline for adding images directly on the place page
...
class Best_Selling_images(models.Model):
    place = models.ForeignKey(Best_Selling, on_delete=models.CASCADE, related_name='images')
    image = models.ImageField(upload_to='place_images/more/')
    caption = models.CharField(max_length=255, blank=True)
